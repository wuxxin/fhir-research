...
def test_marimo_script_generates_image(cleanup_files):
    ...
    convert_process_result = subprocess.run(convert_command, capture_output=True, text=True, cwd=str(PROJECT_ROOT))

    print("Convert STDOUT:", convert_process_result.stdout)
    print("Convert STDERR:", convert_process_result.stderr)
    assert convert_process_result.returncode == 0, f"Marimo convert failed: {convert_process_result.stderr}"
    ...
    process_result = subprocess.run(run_command, capture_output=True, text=True, cwd=str(PROJECT_ROOT), timeout=120) # Added timeout

    print("Execution STDOUT:", process_result.stdout)
    print("Execution STDERR:", process_result.stderr)
    assert process_result.returncode == 0, f"Converted script execution failed: {process_result.stderr}"
