...
def create_observation(
    ...
    observation.subject = FHIRReference.construct(reference=patient_reference_str)

    # Effective DateTime
    try:
        # Attempt to parse as full datetime, assuming Z indicates UTC
        if 'Z' in effective_date_time.upper():
            observation.effectiveDateTime = datetime.datetime.fromisoformat(effective_date_time.replace('Z', '+00:00'))
        # Attempt to parse as date if no time info, or if it's just date
             ...
             observation.effectiveDateTime = datetime.datetime.combine(dt_obj, datetime.time.min) # Add midnight time
        else: # General ISO format with time and potentially timezone
            observation.effectiveDateTime = datetime.datetime.fromisoformat(effective_date_time)

    except ValueError:
        raise ValueError("effective_date_time must be a valid ISO 8601 string (YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS[Z|+/-HH:MM]).")
    ...
    if value_quantity_value is not None and value_quantity_unit is not None:
        observation.valueQuantity = Quantity.construct(
            ...
            code=value_quantity_unit_code if value_quantity_unit_code else value_quantity_unit # Fallback for code
        )
    elif value_cc_system is not None and value_cc_code is not None:
        observation.valueCodeableConcept = CodeableConcept.construct(
            coding=[
                Coding.construct(
                    system=value_cc_system,
                    code=value_cc_code,
                    display=value_cc_display
                )
            ]
        )
...
def create_patient_lab_bundle(
    patient_details: dict,
    lab_observations_details: list
) -> Bundle:
    """
    ...
    for obs_detail in lab_observations_details:
        ...
        observation_resource = create_observation(
            patient_reference_str=patient_reference_str, 
            **obs_detail
        )
    ...
    bundle.timestamp = datetime.datetime.now(datetime.timezone.utc).isoformat() # Use timezone-aware UTC timestamp
...
def flatten_fhir_bundle(bundle_dict: dict) -> pd.DataFrame:
    ...
    if merged_df.empty or merged_df[[col for col in merged_df.columns if col.startswith("patient_")]].isnull().all().all():
        if 'id' in df_patients.columns and 'subject_reference' in df_observations.columns:
            ...
            df_patients_temp['temp_patient_ref'] = "Patient/" + df_patients_temp['id'].astype(str)
    ...
    if merged_df.empty and not df_observations.empty:
        ...
        if not df_patients.empty: # Add empty patient columns if patients exist but didn't merge
    ...
    if 'patient_id' in merged_df.columns and 'id_obs' in merged_df.columns: # Example of a specific cleanup
